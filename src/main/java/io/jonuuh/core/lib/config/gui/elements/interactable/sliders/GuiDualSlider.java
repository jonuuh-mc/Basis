...
//        float w = /*(i == 0) ? */ (leftPointerScreenPos - xPos) /*: (currPointerCenter - getPointerScreenPos(i - 1))*/;
        ...
        // far left
        RenderUtils.drawRoundedRect(GL11.GL_POLYGON, xPos, (yPos + trackHeight), (leftPointerScreenPos - xPos) /*- pOffset*/, trackHeight, parent.getOuterRadius(), parent.getColorMap().get("ACCENT"), true);

        // middle
        RenderUtils.drawRoundedRect(GL11.GL_POLYGON, leftPointerScreenPos /*- pOffset*/, (yPos + trackHeight), (rightPointerScreenPos - leftPointerScreenPos) /*+ (pOffset * 2)*/, trackHeight, parent.getOuterRadius(), parent.getBaseColor(), true);

        // far right
        RenderUtils.drawRoundedRect(GL11.GL_POLYGON, rightPointerScreenPos /*+ pOffset*/, (yPos + trackHeight), width - (rightPointerScreenPos - xPos) /*- pOffset*/, trackHeight, parent.getOuterRadius(), parent.getColorMap().get("ACCENT"), true);
...
//    }
    ...
    protected void drawPointer(boolean isLeftPointer)
    {
        ...
        if (isLeftPointer)
        {
            RenderUtils.drawTriangle(GL11.GL_POLYGON, x /*- (pointerSize / 2F)*/, y, pointerSize, pointerSize, parent.getBaseColor()/*.copy().setA(0.5F)*/, -90);
        }
        else
        {
            RenderUtils.drawTriangle(GL11.GL_POLYGON, x /*+ (pointerSize / 2F)*/, y, pointerSize, pointerSize, parent.getBaseColor()/*.copy().setA(0.5F)*/, 90);
