...
{
    public GuiVerticalSlider(ISettingsGui parent, int xPos, int yPos, int width, int height, double min, double max, double[] startValues)
    {
        super(parent, xPos, yPos, width, height, min, max, startValues);
    }

    public GuiVerticalSlider(ISettingsGui parent, int xPos, int yPos, int width, int height, double min, double max, double startValue)
    {
        super(parent, xPos, yPos, width, height, min, max, startValue);
    }

    public GuiVerticalSlider(ISettingsGui parent, int xPos, int yPos, double min, double max, double[] startValues)
    {
        super(parent, xPos, yPos, min, max, startValues);
    }

    public GuiVerticalSlider(ISettingsGui parent, int xPos, int yPos, double min, double max, double startValue)
    {
...
//            RenderUtils.drawRoundedRect(GL11.GL_POLYGON, trackX, yPos + trackHeight, trackWidth, trackHeight, parent.getOuterRadius(), colors.get(i), true);
            ...
            if (i == normalPointerValues.length - 1)
            {
                ...
                RenderUtils.drawRoundedRect(GL11.GL_POLYGON, xPos + trackWidth, currPointerCenter, trackWidth, height - (currPointerCenter - yPos), parent.getOuterRadius(), colorMap.get("ACCENT").setA(0.8F), true);
            }
        }
